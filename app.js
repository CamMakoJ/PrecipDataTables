...
//Linked to Make Table button on desktop which populates id='bar'
$scope.createTable = function() {
  console.log('updates scope');
};

//the angular filter for years (removes data prior to parsing for Charts.js)
...
$scope.xDataToArray = function (obj) {
  obj.forEach(function(item){
    var time = moment(item.Date, 'MM/DD/YYYY').toObject();
...
$scope.dataZipper = function (x, y) {
  ...
  } else {console.log('mismatched x and y');}
...
$scope.findAveRain = function () {
  ...
  console.log($scope.aveAnnualRain[0].value);
};
